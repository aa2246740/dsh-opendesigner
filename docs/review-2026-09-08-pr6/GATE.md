...
P02 is fail-closed third-state (BLOCKED + USER_REPAIR retained + journal kept), not PR #6’s silent `preserved:true`. Unknown third state must not auto-declare the transaction resolved.
...
Target: 27 main + 8 pr6, all pass. Also `npm test` and existing `test:review*`.
