...
(function (root, factory) {
  ...
  // --- StylesPanel Manager ---
  function StylesPanelManager() {}
  var stylesPanelMethods = {
  ...
  for (var spKey in stylesPanelMethods) {
    if (stylesPanelMethods.hasOwnProperty(spKey)) {
      StylesPanelManager[spKey] = stylesPanelMethods[spKey];
      StylesPanelManager.prototype[spKey] = stylesPanelMethods[spKey];
    }
  }
  ...
  // --- Selection & Snapping Overlay Renderer ---
  function SelectionOverlayRenderer(options) {
    options = options || {};
    this.strokeColor = options.strokeColor || "#3b82f6";
    this.guideColor = options.guideColor || "#ef4444";
    this.handleSize = options.handleSize || 8;
  }
  SelectionOverlayRenderer.prototype.computeHandles = function (box) {
    var left = box.left, top = box.top, width = box.width, height = box.height;
    var midX = left + width / 2;
    var midY = top + height / 2;
    var right = left + width;
    var bottom = top + height;
  ...
  SelectionOverlayRenderer.prototype.renderSvgOverlay = function (box, guides, options) {
    options = options || {};
    guides = guides || [];
    var svgWidth = options.width || 2000;
    var svgHeight = options.height || 2000;
    var parts = [
    ...
    for (var i = 0; i < guides.length; i++) {
      var g = guides[i];
      if (g.orientation === "vertical") {
        parts.push('<line x1="' + g.coordinate + '" y1="' + g.start + '" x2="' + g.coordinate + '" y2="' + g.end + '" stroke="' + this.guideColor + '" stroke-width="1" stroke-dasharray="3 3" />');
        parts.push('<circle cx="' + g.coordinate + '" cy="' + ((g.start + g.end) / 2) + '" r="2" fill="' + this.guideColor + '" />');
      } else {
        parts.push('<line x1="' + g.start + '" y1="' + g.coordinate + '" x2="' + g.end + '" y2="' + g.coordinate + '" stroke="' + this.guideColor + '" stroke-width="1" stroke-dasharray="3 3" />');
        parts.push('<circle cx="' + ((g.start + g.end) / 2) + '" cy="' + g.coordinate + '" r="2" fill="' + this.guideColor + '" />');
      }
    }
    if (box) {
      parts.push('<rect x="' + box.left + '" y="' + box.top + '" width="' + box.width + '" height="' + box.height + '" fill="none" stroke="' + this.strokeColor + '" stroke-width="1.5" />');
      var half = this.handleSize / 2;
      var handles = this.computeHandles(box);
      for (var j = 0; j < handles.length; j++) {
        var h = handles[j];
        parts.push('<rect class="resize-handle handle-' + h.handle + '" x="' + (h.point.x - half) + '" y="' + (h.point.y - half) + '" width="' + this.handleSize + '" height="' + this.handleSize + '" fill="#ffffff" stroke="' + this.strokeColor + '" stroke-width="1.5" style="pointer-events:auto;cursor:' + h.cursor + ';" data-handle="' + h.handle + '" />');
      }
    }
    parts.push('</svg>');
    return parts.join("\n");
  ...
  function CanvasInteractionController(viewport, selection) {
    this.mode = "idle";
    this.viewport = viewport;
    this.selection = selection;
    this.dragSession = null;
    this.currentGuides = [];
    this.onGuidesChangeCallbacks = [];
    this.onModeChangeCallbacks = [];
  }
  CanvasInteractionController.prototype.getMode = function () { return this.mode; };
  CanvasInteractionController.prototype.getGuides = function () { return this.currentGuides.slice(); };
  CanvasInteractionController.prototype.onGuidesChange = function (cb) {
    var self = this;
    this.onGuidesChangeCallbacks.push(cb);
    return function () { self.onGuidesChangeCallbacks = self.onGuidesChangeCallbacks.filter(function (c) { return c !== cb; }); };
  };
  CanvasInteractionController.prototype.onModeChange = function (cb) {
    var self = this;
    this.onModeChangeCallbacks.push(cb);
    return function () { self.onModeChangeCallbacks = self.onModeChangeCallbacks.filter(function (c) { return c !== cb; }); };
  };
  CanvasInteractionController.prototype.setMode = function (nextMode) {
    if (this.mode === nextMode) return;
    this.mode = nextMode;
    for (var i = 0; i < this.onModeChangeCallbacks.length; i++) this.onModeChangeCallbacks[i](nextMode);
  };
  CanvasInteractionController.prototype.emitGuides = function (guides) {
    this.currentGuides = guides;
    for (var i = 0; i < this.onGuidesChangeCallbacks.length; i++) this.onGuidesChangeCallbacks[i](guides);
  };
  CanvasInteractionController.prototype.startPan = function (screenPoint) {
    this.setMode("panning");
    this.dragSession = { startScreenPoint: { x: screenPoint.x, y: screenPoint.y } };
  };
  CanvasInteractionController.prototype.updatePan = function (screenPoint) {
    if (this.mode !== "panning" || !this.dragSession) return;
    var dx = screenPoint.x - this.dragSession.startScreenPoint.x;
    var dy = screenPoint.y - this.dragSession.startScreenPoint.y;
    this.viewport.pan(dx, dy);
    this.dragSession.startScreenPoint = { x: screenPoint.x, y: screenPoint.y };
  };
  CanvasInteractionController.prototype.endPan = function () {
    if (this.mode === "panning") {
      this.dragSession = null;
      this.setMode("idle");
    }
  };
  CanvasInteractionController.prototype.startDrag = function (screenPoint) {
    var box = this.selection.getBoundingBox();
    if (!box) return false;
    var worldP = this.viewport.toWorld(screenPoint);
    this.dragSession = {
      startScreenPoint: { x: screenPoint.x, y: screenPoint.y },
      startWorldPoint: worldP,
      initialBox: { left: box.left, top: box.top, width: box.width, height: box.height }
    };
    this.setMode("dragging");
    return true;
  };
  CanvasInteractionController.prototype.updateDrag = function (screenPoint, options) {
    if (this.mode !== "dragging" || !this.dragSession) return null;
    var currentWorld = this.viewport.toWorld(screenPoint);
    var totalDx = currentWorld.x - this.dragSession.startWorldPoint.x;
    var totalDy = currentWorld.y - this.dragSession.startWorldPoint.y;
    var currentBox = this.selection.getBoundingBox();
    if (!currentBox) return null;
    var targetLeft = this.dragSession.initialBox.left + totalDx;
    var targetTop = this.dragSession.initialBox.top + totalDy;
    var stepDx = targetLeft - currentBox.left;
    var stepDy = targetTop - currentBox.top;
    var moveRes = this.selection.moveSelection(stepDx, stepDy, options);
  ...
  CanvasInteractionController.prototype.endDrag = function () {
  ...
  CanvasInteractionController.prototype.startResize = function (handle, screenPoint) {
    var worldP = this.viewport.toWorld(screenPoint);
    var ok = this.selection.startResize(handle, worldP);
    if (ok) this.setMode("resizing");
    return ok;
  };
  CanvasInteractionController.prototype.updateResize = function (screenPoint, options) {
    if (this.mode !== "resizing") return null;
    var currentWorld = this.viewport.toWorld(screenPoint);
    var resizeRes = this.selection.updateResize(currentWorld, options);
  ...
  CanvasInteractionController.prototype.endResize = function () {
  ...
  // --- FlatStore (Client-side lightweight) ---
  function FlatStore() {
    this.byId = new Map();
    this.childrenByParent = new Map();
    this.parentByChild = new Map();
  }
  FlatStore.prototype.setElement = function (el) {
    this.byId.set(el.id, el);
  };
  FlatStore.prototype.getElement = function (id) {
    return this.byId.get(id);
  };
  FlatStore.prototype.removeElement = function (id) {
    this.byId.delete(id);
    this.parentByChild.delete(id);
    this.childrenByParent.delete(id);
  };
  FlatStore.prototype.getRootIds = function () {
    var roots = [];
    for (var k of this.byId.keys()) {
      if (!this.parentByChild.has(k)) roots.push(k);
    }
    return roots;
  };
  ...
  function CanvasPanel(options) {
    ...
    this.viewport = new InfiniteCanvasViewport({
      zoom: options.initialZoom !== undefined ? options.initialZoom : 1.0,
      panX: options.initialPan ? options.initialPan.x : 0,
      panY: options.initialPan ? options.initialPan.y : 0
    });
    ...
    this.snapThreshold = options.snapThreshold !== undefined ? options.snapThreshold : 5;
    this.elementRects = new Map();
  }
  CanvasPanel.prototype.registerElement = function (id, rect, element) {
    this.elementRects.set(id, { left: rect.left, top: rect.top, width: rect.width, height: rect.height });
    if (element) {
      if (!element.canvasRect) {
        element.canvasRect = { left: rect.left, top: rect.top, width: rect.width, height: rect.height };
      }
      this.store.setElement(element);
    }
    this.syncSelectionManager();
  };
  CanvasPanel.prototype.unregisterElement = function (id) {
    this.elementRects.delete(id);
    this.store.removeElement(id);
    this.syncSelectionManager();
  };
  CanvasPanel.prototype.syncSelectionManager = function () {
    var list = [];
    for (var entry of this.elementRects.entries()) {
      list.push({ id: entry[0], rect: { left: entry[1].left, top: entry[1].top, width: entry[1].width, height: entry[1].height } });
    }
    this.selection.setElements(list);
  };
  CanvasPanel.prototype.getElementRect = function (id) {
    var r = this.elementRects.get(id);
    return r ? { left: r.left, top: r.top, width: r.width, height: r.height } : undefined;
  };
  CanvasPanel.prototype.getAllElementRects = function () {
    return new Map(this.elementRects);
  };
  CanvasPanel.prototype.getAlignmentCandidates = function () {
    var selected = new Set(this.selection.getSelectedIds());
    var candidates = [];
    for (var entry of this.elementRects.entries()) {
      if (!selected.has(entry[0])) {
        candidates.push({ left: entry[1].left, top: entry[1].top, width: entry[1].width, height: entry[1].height });
      }
    }
    return candidates;
  };
  CanvasPanel.prototype.select = function (ids) {
    this.selection.select(ids);
  };
  CanvasPanel.prototype.getSelectedBoundingBox = function () {
    return this.selection.getBoundingBox();
  };
  CanvasPanel.prototype.moveSelected = function (screenPoint) {
    var candidates = this.getAlignmentCandidates();
    var res = this.controller.updateDrag(screenPoint, {
      candidates: candidates,
      snapThreshold: this.snapThreshold,
      enableSnapping: true
    });
    if (res && res.updatedElements) {
      for (var i = 0; i < res.updatedElements.length; i++) {
        var item = res.updatedElements[i];
        this.elementRects.set(item.id, { left: item.rect.left, top: item.rect.top, width: item.rect.width, height: item.rect.height });
        var el = this.store.getElement(item.id);
        if (el) el.canvasRect = { left: item.rect.left, top: item.rect.top, width: item.rect.width, height: item.rect.height };
      }
    }
    return res;
  };
  CanvasPanel.prototype.resizeSelected = function (screenPoint) {
    var candidates = this.getAlignmentCandidates();
    var res = this.controller.updateResize(screenPoint, {
      candidates: candidates,
      snapThreshold: this.snapThreshold,
      enableSnapping: true
    });
  ...
  CanvasPanel.prototype.renderHtml = function () {
    var transformStyle = this.viewport.getCssTransform();
    var box = this.selection.getBoundingBox();
    var guides = this.controller.getGuides();
    var elementsHtml = [];
    var rootIds = this.store.getRootIds();
    for (var i = 0; i < rootIds.length; i++) {
      elementsHtml.push(this.sandbox.renderToHtml(this.store, rootIds[i]));
    }
    var overlaySvg = this.overlay.renderSvgOverlay(box, guides);
    return [
  ...
  function initDesignerClient() {
    ...
    var panel = new CanvasPanel({ store: store });
    return {
  ...
  var clientExports = {
    ...
    CanvasInteractionController: CanvasInteractionController,
