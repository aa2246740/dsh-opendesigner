    ...
    renderSvgOverlay(box, guides = [], options = {}) {
        ...
        for (const guide of guides) {
            if (guide.orientation === "vertical") {
                parts.push(`<line x1="${guide.coordinate}" y1="${guide.start}" x2="${guide.coordinate}" y2="${guide.end}" stroke="${this.guideColor}" stroke-width="1" stroke-dasharray="3 3" />`);
                parts.push(`<circle cx="${guide.coordinate}" cy="${(guide.start + guide.end) / 2}" r="2" fill="${this.guideColor}" />`);
            }
            else {
                parts.push(`<line x1="${guide.start}" y1="${guide.coordinate}" x2="${guide.end}" y2="${guide.coordinate}" stroke="${this.guideColor}" stroke-width="1" stroke-dasharray="3 3" />`);
                parts.push(`<circle cx="${(guide.start + guide.end) / 2}" cy="${guide.coordinate}" r="2" fill="${this.guideColor}" />`);
            }
        }
        ...
        if (box) {
            ...
            for (const h of handles) {
                ...
                parts.push(`<g class="resize-handle-group" data-handle="${h.handle}" data-testid="overlay-handle-${h.handle}" data-tooltip="${tip}" style="pointer-events:auto;cursor:${h.cursor};">
            <title>${tip}</title>
        ...
        if (marquee && (marquee.width > 1 || marquee.height > 1)) {
            parts.push(`<rect class="marquee-band" data-testid="marquee-band" x="${marquee.left}" y="${marquee.top}" width="${marquee.width}" height="${marquee.height}" fill="rgba(96,165,250,0.16)" stroke="#60a5fa" stroke-width="1" stroke-dasharray="4 3" />`);
        }
        parts.push(`</svg>`);
        return parts.join("\n");
    }
