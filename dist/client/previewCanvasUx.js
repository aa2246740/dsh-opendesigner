...
export function applyRectsToDom(canvasEl, panel, store) {
...
export function refreshOverlay(canvasEl, panel) {
    const host = canvasEl.querySelector(".canvas-overlay-host");
    if (host) {
        host.innerHTML = panel.overlay.renderSvgOverlay(panel.getSelectedBoundingBox(), panel.controller.getGuides(), { marquee: panel.controller.getMarqueeRect() });
    }
...
function endInteraction(panel) {
    ...
    if (mode === "panning")
        panel.controller.endPan();
    else if (mode === "dragging")
        panel.controller.endDrag();
    else if (mode === "resizing")
        panel.controller.endResize();
    else if (mode === "box-selecting")
        panel.controller.endBoxSelect();
}
export function bindFloatingTooltips(root) {
    ...
    const onMove = (event) => {
        ...
        if (el)
            show(el.getAttribute("data-tooltip") || "", event.clientX, event.clientY);
    };
    const onOut = (event) => {
        const next = event.relatedTarget;
        if (next && root.contains(next) && next.closest("[data-tooltip]"))
            return;
        hide();
    };
...
export function bindPreviewCanvasUx(canvasEl, panel, store, hooks) {
    let spaceDown = false;
    let pointerId = null;
    let didMutate = false;
...
//# sourceMappingURL=previewCanvasUx.js.map