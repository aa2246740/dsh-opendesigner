...
import { mergeTailwindClasses } from "../compiler/tailwindMerge.js";
import { bindPreviewCanvasUx, bindFloatingTooltips, refreshOverlay } from "./previewCanvasUx.js";
export function autosaveClearsDirty(httpOk, result, expected) {
    if (!httpOk || result?.success === false)
        return false;
    if (expected?.localEditId !== undefined && result?.localEditId !== expected.localEditId)
        return false;
    if (expected?.ackRevision !== undefined && result?.ackRevision !== expected.ackRevision)
        return false;
    return true;
}
...
export function mountPreview(root, api = {}) {
    ...
    <div class="od-shell">
      ...
      <div class="od-main">
        ...
        <aside class="od-styles">
          ...
          <textarea id="od-intent" class="od-intent" data-testid="ai-intent" rows="3" placeholder="例如：把按钮改成翡绿 / make the button emerald"></textarea>
          <div class="od-actions">
  ...
  `;
    ...
    function render(options = {}) {
        ...
        syncGeometry();
        if (selected.length)
            panel.select(selected.filter((id) => store.getElement(id)));
    ...
    async function refreshStatus() {
        ...
        if (openBatchId && api.callTool) {
            ...
            if (Array.isArray(diffs.diffs))
                fileDiffCount = diffs.diffs.length;
            else if (Array.isArray(diffs.copied))
                fileDiffCount = diffs.copied.length;
        }
        syncProposalButtons();
    }
    ...
    async function callTool(tool, args = {}) {
        ...
        if (tool === "checkpoint" || tool === "autosave" || tool === "apply_to_project") {
            ...
            if (!pushed)
                return { success: false, error: "STALE_HYDRATE" };
        }
        const result = await api.callTool(tool, args);
        const shown = { ...result };
        if (shown.store)
            shown.store = { restored: true };
    ...
    function insertBox() {
        let n = 1;
        while (store.getElement(`insert-box-${n}`))
            n += 1;
    ...
    inspectorEl.addEventListener("click", (event) => {
        ...
        if (fill)
            applyStyle("backgroundColor", fill, `fill-${fill}`);
        else if (radius)
            applyStyle("borderRadius", radius, `radius-${radius}`);
        else if (padding)
            applyStyle("padding", padding, `padding-${padding}`);
        else if (text)
            applyStyle("textColor", text, `text-${text}`);
        else if (shadow)
            applyLocalShadow(shadow);
    });
        ...
        try {
            ...
            if (api.getCanvas) {
                ...
                if (canvas && canvas.byId && Object.keys(canvas.byId).length > 0) {
                    ...
                    if (typeof canvas.version === "number")
                        lastSeenVersion = canvas.version;
                    if (typeof canvas.projectId === "string")
    ...
    const boot = () => {
        ...
        mountPreview(host, {
            ...
            getCanvas: async () => {
                ...
                if (!res.ok)
                    throw new Error(`canvas HTTP ${res.status}`);
                return await res.json();
            },
