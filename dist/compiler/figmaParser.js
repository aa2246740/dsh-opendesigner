 ...
 */
export function figmaColorToTailwind(color, prefix = "bg") {
    ...
    if (r === 255 && g === 255 && b === 255)
        return `${prefix}-white${opacitySuffix}`;
    if (r === 0 && g === 0 && b === 0)
        return `${prefix}-black${opacitySuffix}`;
    if (a === 0)
        return `${prefix}-transparent`;
    // 近似 Tailwind 调色盘
    if (r > 200 && g < 100 && b < 100)
        return `${prefix}-red-500${opacitySuffix}`;
    if (r < 100 && g > 180 && b < 100)
        return `${prefix}-green-500${opacitySuffix}`;
    if (r < 100 && g < 150 && b > 220)
        return `${prefix}-blue-500${opacitySuffix}`;
    if (r > 200 && g > 150 && b < 50)
        return `${prefix}-amber-500${opacitySuffix}`;
    if (r > 100 && g < 100 && b > 200)
        return `${prefix}-indigo-500${opacitySuffix}`;
    if (r > 200 && g < 100 && b > 200)
        return `${prefix}-pink-500${opacitySuffix}`;
    // 灰阶检测
    if (Math.abs(r - g) < 10 && Math.abs(g - b) < 10) {
        if (r > 230)
            return `${prefix}-gray-100${opacitySuffix}`;
        if (r > 200)
            return `${prefix}-gray-200${opacitySuffix}`;
        if (r > 150)
            return `${prefix}-gray-400${opacitySuffix}`;
        if (r > 100)
            return `${prefix}-gray-600${opacitySuffix}`;
        if (r > 40)
            return `${prefix}-gray-800${opacitySuffix}`;
 ...
 */
export function pixelToTailwindScale(px) {
 ...
 */
export function figmaNodeToTailwindClasses(node) {
    const classes = [];
    ...
    if (node.stackMode === "HORIZONTAL") {
        ...
        else if (node.counterAxisAlignItems === "STRETCH")
    ...
    if (paddingLeft !== undefined && paddingLeft === paddingRight && paddingLeft === paddingTop && paddingLeft === paddingBottom) {
        ...
        if (paddingLeft !== undefined && paddingLeft === paddingRight) {
            ...
            if (paddingLeft)
                classes.push(`pl-${pixelToTailwindScale(paddingLeft)}`);
            if (paddingRight)
                classes.push(`pr-${pixelToTailwindScale(paddingRight)}`);
        }
        if (paddingTop !== undefined && paddingTop === paddingBottom) {
            ...
            if (paddingTop)
                classes.push(`pt-${pixelToTailwindScale(paddingTop)}`);
            if (paddingBottom)
                classes.push(`pb-${pixelToTailwindScale(paddingBottom)}`);
        }
    }
    // 3. Fills (背景色与渐变)
    if (node.fills && node.fills.length > 0) {
        ...
        if (gradient) {
            ...
            if (Array.isArray(stops) && stops.length >= 2) {
                classes.push(figmaColorToTailwind(stops[0].color, "from"), figmaColorToTailwind(stops[stops.length - 1].color, "to"));
            }
            else {
    ...
    if (node.type === "TEXT") {
        ...
        if (node.lineHeight) {
            ...
            if (lhMap[lh])
                classes.push(lhMap[lh]);
            else
                classes.push(`leading-[${lh}px]`);
        }
        // 文字颜色提取自 fills
        if (node.fills && node.fills.length > 0) {
 ...
 */
export function buildSceneIndex(nodes) {
 ...
 */
export function packKiwiBinary(nodes, magic = "figma") {
    const writer = new KiwiWriter();
    writer.writeHeader(magic);
    const typeMapInverse = {
        FRAME: 1,
        GROUP: 2,
        TEXT: 3,
        RECTANGLE: 4,
    ...
    const primaryMapInverse = {
        MIN: 1,
        CENTER: 2,
        MAX: 3,
        SPACE_BETWEEN: 4
    };
    const counterMapInverse = {
        MIN: 1,
        CENTER: 2,
        MAX: 3,
        BASELINE: 4,
 ...
 */
export function unpackKiwiBinary(buffer, options = {}) {
    ...
    while (!reader.isEOF()) {
        try {
            ...
            if (autoLayoutMode > 0) {
                ...
                if (primaryMap[primTag])
                    node.primaryAxisAlignItems = primaryMap[primTag];
                const counterTag = reader.readByte();
                if (counterMap[counterTag])
                    node.counterAxisAlignItems = counterMap[counterTag];
            }
            // 读取色彩 Fill
            ...
            if (node.type === "TEXT") {
                ...
                if (textAlignMap[alignTag])
                    node.textAlignHorizontal = textAlignMap[alignTag];
 ...
 */
export function sceneToFlatStore(scene, targetStore, targetParentId) {
    const { nodeMap, childrenMap, rootIds } = scene;
    const elements = [];
    const visited = new Set();
    const mainRootId = rootIds[0] || "figma_root";
    function traverse(nodeId, parentId) {
        ...
        if (fNode.type === "TEXT")
            tag = "p";
        else if (fNode.name.toLowerCase().includes("button"))
            tag = "button";
        else if (fNode.name.toLowerCase().includes("input"))
            tag = "input";
        else if (fNode.name.toLowerCase().includes("card"))
            tag = "section";
        else if (fNode.name.toLowerCase().includes("nav"))
            tag = "nav";
        else if (fNode.name.toLowerCase().includes("header"))
            tag = "header";
        else if (fNode.name.toLowerCase().includes("footer"))
            tag = "footer";
        else if (fNode.name.toLowerCase().includes("aside"))
            tag = "aside";
        const el = {
            id: fNode.id,
            type: "element",
    ...
    const jsxVisited = new Set();
    function buildJsx(nodeId, indent = 0) {
        if (jsxVisited.has(nodeId))
            return "";
 ...
 */
export function parseFigmaClipboard(input) {
    try {
        let nodes = [];
        if (input instanceof Uint8Array) {
            ...
            if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
                try {
                    ...
                    const flattenTree = (root, parentId) => {
                    ...
                    if (Array.isArray(parsed)) {
                        ...
                        nodes = parsed.nodes.flatMap((item) => flattenTree(item));
                    }
                    else {
 ...
 */
export function figmaToReact19(input, options = {}) {
    let scene;
    let rawElements = [];
    let rootId = "FigmaRoot";
    if (input instanceof Uint8Array || typeof input === "string") {
        ...
        if (parsed.scene) {
            ...
            const nodes = rawElements.map((el) => ({
                id: el.id,
                name: el.tag,
                type: el.tag === "p" ? "TEXT" : "FRAME",
                characters: el.textContent
    ...
    else if ("elements" in input && Array.isArray(input.elements)) {
        const p = input;
    ...
    function renderJsxNode(nodeId, indent = 2) {
        if (jsxVisited.has(nodeId))
            return "";
        jsxVisited.add(nodeId);
