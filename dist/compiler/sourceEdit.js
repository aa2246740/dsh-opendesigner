...
import { mergeTailwindTokens } from "./tailwindMerge.js";
export { getTailwindCategory, mergeTailwindTokens, mergeTailwindClasses, dropTailwindCategory } from "./tailwindMerge.js";
/**
 * 基于精确切片区间的 className 更新
 */
export function updateClassNameDeterministically(sourceCode, classStartOffset, classEndOffset, updatedClassLiteral) {
    try {
 ...
 */
export function findBestMatchingOpeningElement(ast, targetLine, targetColumn) {
 ...
 */
export function updateSourceCodeDeterministically(request) {
    ...
    if (newClassName !== undefined) {
        const classAttr = targetOpeningNode.attributes.find((attr) => attr.type === "JSXAttribute" && attr.name && attr.name.name === "className");
        if (classAttr) {
            // 如果已有 className
            ...
            const res = updateClassNameDeterministically(workingCode, classAttr.value.start, classAttr.value.end, replacement);
            if (!res.ok)
                return res;
    ...
    if (newStyleProp !== undefined) {
        ...
        const styleAttr = freshNode.attributes.find((attr) => attr.type === "JSXAttribute" && attr.name && attr.name.name === "style");
        if (styleAttr) {
            if (!styleAttr.value ||
            ...
            const existingProp = objExpr.properties.find((p) => p.type === "ObjectProperty" && (p.key.name === key || p.key.value === key));
            if (existingProp) {
                workingCode =
...
//# sourceMappingURL=sourceEdit.js.map