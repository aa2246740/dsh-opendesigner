...
export class ASTParser {
    ...
    parseJSXMemberOrIdentifier() {
        let expr = this.parseJSXIdentifier();
        while (this.peek(0) === ".") {
    ...
    parseJSXElement() {
        ...
        if (!openingElement.selfClosing) {
            while (this.pos < this.length) {
                ...
                if (this.peek(0) === "<" && this.peek(1) === "/") {
                    ...
                    if (openName !== closeName) {
                        this.raiseError(`Mismatched JSX tags: expected </${openName}>, found </${closeName}>`, closingElement.start);
                    }
                    break;
                }
                // 嵌套 JSX 元素
            ...
            if (!closingElement) {
                this.raiseError(`Unclosed JSX element <${openingElement.name.name}>`, start);
            }
        }
...
export function traverseAST(ast, visitor, parent = null) {
    const path = { node: ast, parent };
    if (visitor.enter) {
        visitor.enter(path);
    }
