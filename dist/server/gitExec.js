...
export async function git(cwd, args, extraEnv = {}) {
...
export async function gitBytes(cwd, args, extraEnv = {}) {
    return await execFileAsync("git", args, {
        cwd,
        encoding: "buffer",
        env: { ...GIT_ISOLATED_ENV, ...extraEnv },
...
export async function isGitRoot(cwd) {
    try {
...
export async function gitShowBytes(cwd, ref, rel) {
    try {
        ...
        if (/does not exist|exists on disk, but not in|bad revision|is outside repository|not in the/i.test(combined)) {
            return { kind: "absent" };
        }
