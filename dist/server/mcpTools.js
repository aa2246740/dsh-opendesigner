...
import { applySurgicalEdits } from "../compiler/aiMerge.js";
import { assertDestructiveApproval } from "./approval.js";
import { snapshotJsxAsSvgDataUrl } from "./jsxSnapshot.js";
import { resolveProjectPath } from "./pathJail.js";
import { copyFileNoFollow, readTextNoFollow, unlinkNoFollow, writeFileNoFollow } from "./fileBytes.js";
export const OPEN_DESIGNER_TOOLS = [
 ...
 */
export function elementToJSX(store, elementId) {
    ...
    switch (toolName) {
        ...
        case "project_grep": {
            ...
            for (const rel of files) {
                if (args.pathPrefix && !rel.startsWith(args.pathPrefix))
                    continue;
                try {
        ...
        case "canvas_claim": {
            ...
            const res = claims.claim(elementId, covering_hash, {
                ...
                isRelated: (lockedId, requestedId) => store.isDescendant(lockedId, requestedId) || store.isDescendant(requestedId, lockedId)
            });
            return res;
        }
        case "canvas_release": {
        ...
        case "canvas_update": {
            ...
            const isDescendant = (anc, tgt) => store.isDescendant(anc, tgt);
        ...
        case "canvas_query": {
            ...
            const matches = [];
            const json = store.toJSON();
            for (const el of Object.values(json.byId)) {
        ...
        case "take_screenshot": {
            ...
            if (ctx.captureScreenshot) {
                ...
                const { ComponentSandbox } = await import("../client/sandbox.js");
