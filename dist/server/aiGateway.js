 ...
 */
import { SAVE_TO_CODE_JSON_SCHEMA, applySurgicalEdits } from "../compiler/aiMerge.js";
 ...
 */
export function detectLiveProvidersFromEnv(env = process.env) {
    const out = [];
...
export function detectAiConfigFromEnv(env = process.env) {
...
export function liveProvidersStatus(env = process.env) {
...
export class AIGateway {
    ...
    constructor(config = {}) {
        ...
        this.fetchFn = config.fetchFn || (typeof globalThis.fetch === "function" ? globalThis.fetch.bind(globalThis) : (async () => { }));
     ...
     */
    async generateAndApply(req, options = {}) {
     ...
     */
    async requestEdits(req, feedbackError) {
        // 离线/测试模拟模式
        if (this.mockMode) {
        ...
        if (!response.ok) {
            ...
            const retryAfterMs = response.status === 429 || response.status === 503 ? parseRetryAfterMs(response) : undefined;
            return {
                success: false,
        ...
        while ((cbMatch = codeBlockRegex.exec(content)) !== null) {
            if (cbMatch[1]?.trim())
                candidateJsonStrings.push(cbMatch[1].trim());
        }
        if (candidateJsonStrings.length === 0) {
            ...
            if (editsObjIdx !== -1) {
                ...
                if (endIdx !== -1)
                    candidateJsonStrings.push(slice.slice(0, endIdx));
            }
            // 尝试提取 [{"old_string": ...}]
            ...
            if (fallbackMatch)
                candidateJsonStrings.push(fallbackMatch[0].trim());
        }
     ...
     */
    generateMockEdits(req, feedbackError) {
...
export async function generateAndApplyLive(req, options = {}) {
